<template>
  <nav class="relative flex gap-2 justify-between items-center content-between p-1 z-index-10 md:hidden w-full bg-gray-100 dark:bg-darkest">
    <router-link class="p-1.5 text-lg whitespace-no-wrap flex justify-between items-center" :to="{ path: '/docs/new' }">
    ...
    <div class="flex gap-1 items-stretch">
      ...
      <router-link v-if="isDashboard" :to="{ path: '/quick-action' }" class="button-flat button-size-medium">
        <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      </router-link>
      <router-link v-else-if="isDoc" :to="{ path: `/docs/${doc.id}/meta` }" class="button-flat button-size-medium">
        <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
