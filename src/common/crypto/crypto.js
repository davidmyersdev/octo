import asym from '#root/src/common/crypto/asymmetric'
import sym from '#root/src/common/crypto/symmetric'
import { pack, unpack } from '#root/src/common/crypto/utils'

// decrypt the data key with our private key
const unwrapKey = async (wrappedKey, privateKey) => {
  return globalThis.crypto.subtle.unwrapKey('raw', wrappedKey, privateKey, asym.algorithm, sym.algorithm, true, ['encrypt', 'decrypt'])
}

// encrypt the data key with our public key
const wrapKey = async (key, publicKey) => {
  return globalThis.crypto.subtle.wrapKey('raw', key, publicKey, asym.algorithm)
}
