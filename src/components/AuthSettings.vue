<template>
  <section class="bg-gray-100 dark:bg-darkest shadow rounded p-4">
    <h4 class="text-2xl text-brand mb-2">{{ user ? 'Account' : 'Sync Docs' }}</h4>
    ...
    <div v-if="user" class="lg:flex">
      ...
      <div class="flex-grow">
        <h5 class="text-xl mb-2">Details</h5>
        <div class="mb-4">
        ...
        <div class="flex items-center">
          <button @click="signOut" class="button button-size-medium button-color-surface">
            <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button v-if="!github" @click="linkGithub" class="button button-size-medium button-color-surface ml-2">
            <GitHubIcon />
            <span class="action ml-3">Link GitHub</span>
          </button>

          <button v-if="!twitter" @click="linkTwitter" class="button button-size-medium button-color-surface ml-2">
            <TwitterIcon />
            <span class="action ml-3">Link Twitter</span>
          </button>

          <button v-if="!google" @click="linkGoogle" class="button button-size-medium button-color-surface ml-2">
            <GoogleIcon />
    ...
    <div v-else>
      ...
      <div class="mb-4">
        <p class="mb-4">Sign up for an account in less than a minute and enjoy the following benefits for free.</p>
        <ul class="text-brand mb-4">
          <li class="flex items-center mb-2">
            <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
            </svg>
            <span class="ml-3">Privacy-focused and zero cookies</span>
          </li>
          <li class="flex items-center mb-2">
            <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
            </svg>
            <span class="ml-3">Back up, sync, and share your documents with Firebase</span>
          </li>
          <li class="flex items-center mb-2">
            <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <p class="mb-2">You can sign in with the following providers.</p>
        <p v-if="!online" class="text-muted">You are currently offline.</p>
      </div>

      <div class="flex text-brand">
        <button @click="signInGithub" :disabled="!online" class="button button-size-medium button-color-surface shadow">
          <GitHubIcon />
          <span class="action ml-3">GitHub</span>
        </button>
        <button @click="signInTwitter" :disabled="!online" class="button button-size-medium button-color-surface ml-2">
          <TwitterIcon />
          <span class="action ml-3">Twitter</span>
        </button>
        <button @click="signInGoogle" :disabled="!online" class="button button-size-medium button-color-surface ml-2">
          <GoogleIcon />
          <span class="action ml-3">Google</span>
        </button>
      </div>
    </div>
  </section>
