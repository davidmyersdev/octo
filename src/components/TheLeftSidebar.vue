    ...
    <div class="side-nav-spacer flex flex-col flex-grow pb-3">
      <div class="other-actions">
        ...
        <router-link :to="{ name: 'account' }" class="sidebar-link">
          ...
          <span class="action flex flex-grow items-stretch justify-between ml-6 md:ml-3">
            <span>{{ authIsEvaluated && !user ? 'Sign In to Account' : 'Account' }}</span>
          </span>
        </router-link>
        <router-link v-if="authIsEvaluated && !user" @click.native="trackCta" :to="{ name: 'account' }" class="sidebar-link text-theme">
          <svg height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
